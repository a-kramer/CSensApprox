...
typedef enum {GSL,CVODES} solver_choice;
...
/* If no simulation time span was specified, it is inferred from the
   ...
   If tspan is all zeros, then it is corrected to 100 points between `0` and
   `1.3*max(t)` */
void fix_tspan_if_necessary(ndarray *fixed_tspan, gsl_vector *t, /* measurement time vector (from the data file) */
 ndarray *user_tspan) /* 3-element tspan vector to fix */
{
  assert(user_tspan);
  assert(fixed_tspan);
  assert(fixed_tspan->size[0]==3);
  int i=0;
  double tf=gsl_vector_max(t)*1.3; // a reasonable stop time [default value]
  double *a,*b=NULL,*c;
  ndarray *tspan=user_tspan;
  a=ndarray_ptr(tspan,&i);
  if (tspan->size[0]>=3) {
    i++;
    b=ndarray_ptr(fixed_tspan,&i);
  }
  i++;
...
main(int argc, char *argv[]){
  ...
  for (i=1;i<argc;i++){
    ...
    } else if (option_is("-c","--cvodes",argv[i])){
      c=CVODES;
